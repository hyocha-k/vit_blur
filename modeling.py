...
def STEFunction(inputs, threshold):
    ...
    quantized = quantized * torch.sign(output)
...
class Attention(nn.Module):
    def __init__(self, config, vis):
        ...
        # Initialize sigma as a learnable parameter
        self.sigma = nn.Parameter(torch.ones(1) * 1.0)  # starting value of 1.0, but it's learnable now     

        # Initialize threshold as a buffer or parameter
        # self.register_buffer('threshold', torch.tensor([init_threshold])) 
        # Or, if you want it to be a trainable parameter:
    ...
    def apply_gaussian_blur(self, attention_probs, kernel_size):
        ...
        gaussian_kernel = gaussian_kernel.to(attention_probs.device)

        # Adjust kernel for a single channel
        gaussian_kernel = gaussian_kernel.repeat(1, 1, 1, 1)

        # Reshape attention_probs
    ...
    def create_gaussian_kernel(kernel_size, sigma):
        ...
        sigma = sigma.view(1)  # or sigma = sigma.unsqueeze(0)

        # Generate a 1D Gaussian kernel
        x = torch.arange(kernel_size).float().to(sigma.device) - (kernel_size // 2)
        gaussian_1d = torch.exp(-x.pow(2) / (2 * sigma.pow(2)))
    ...
    def forward(self, hidden_states):
        ...
        # attention_probs_blurred = self.apply_gaussian_blur(attention_scores, threshold=THRESHOLD, kernel_size=3)
        attention_probs_blurred = self.apply_gaussian_blur(attention_scores, kernel_size=3)
        
        attention_scores = attention_probs_blurred
        attention_probs = self.softmax(attention_scores)

        # attention_probs = attention_probs_blurred
